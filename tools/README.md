...
case; no other procedure may evaluate a formula. Each formula is built only from
literals, the serial text `S` and `CStr` of numbers, so it can be inspected. The oracle may
