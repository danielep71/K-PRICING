...
3. For template gate/workflow/runtime changes, run checker-development and
   policy-coverage checks plus the initializer fixtures for all three profiles.
   Reusable-workflow changes additionally require the hosted consumer fixtures
   and specialist boundary checks specified in its authority. In a generated
   repository, run the applicable retained gates and its own specialist jobs;
   do not restore template-only tools merely to claim a template-level pass.
...
6. Merge manually through the protected workflow. Keep repository automatic
